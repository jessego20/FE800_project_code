...
class MarkovSwitchingModel:
    ...
    def _initialize_from_data(self, returns: np.ndarray) -> None:
        ...
        for k, regime_indices in enumerate(splits):
            if len(regime_indices) > 1:
                ...
                self.stds[k] = max(np.std(regime_returns), 0.001)  # Avoid zero std
                ...
                if len(regime_indices) > 2:
                    ...
                    x = regime_returns[:-1]
                    if np.std(x) > 0:
                        beta_hat = np.corrcoef(x, y)[0,1] * (np.std(y) / np.std(x))
                        self.betas[k] = beta_hat * 0.1
                    else:
                        self.betas[k] = 0.0
                else:
                    self.betas[k] = 0.0
            else:
                # Fallback for regimes with very few observations
        ...
        # Initialize transition matrix with strong self-persistence
        self.transition_probs = np.zeros((self.n_regimes, self.n_regimes))
        for i in range(self.n_regimes):
            self.transition_probs[i, i] = 0.8
            other_prob = 0.2 / (self.n_regimes - 1) if self.n_regimes > 1 else 0
            for j in range(self.n_regimes):
                if i != j:
                    self.transition_probs[i, j] = other_prob
    ...
    def analyze_results(self, data: Optional[pd.DataFrame] = None, 
        ...
        # Parameter estimates - dynamic for n_regimes
        ...
        # for i in range(self.n_regimes):
        #     regime_mask = estimated_regimes == i
        #     if regime_mask.sum() > 0:
        #         regime_returns = data['returns'][regime_mask]
        #         vol = regime_returns.std() * np.sqrt(252)
        #         print(f"Regime {i} - Mean: {regime_returns.mean():.6f}, Vol: {vol:.2%}")
    ...
    def plot_results(self, data: Optional[pd.DataFrame] = None, 
        ...
        # Plot 4: Return distributions by regime
        ...
        # Plot 5: Parameter posteriors for means
        if self.mcmc_samples is not None:
            ...
            for i in range(self.n_regimes):
                ...
                if true_params is not None:
                    ax5.axvline(true_params['means'][i], color=colors[i], linestyle='--', 
                            label=f'True μ_{i}')
    ...
    def diagnostics(self, data: Optional[pd.DataFrame] = None,
        ...
        for t in range(1, len(returns)):
            ...
            ll = norm.logpdf(returns[t], mean, self.stds[regime])
            log_likelihood += ll
        ...
        if self.mcmc_samples is not None:
            ...
            if trace_plots:
                # Trace plots - dynamic grid
                n_params = self.n_regimes
                fig, axes = plt.subplots(3, n_params, figsize=(5*n_params, 10))
